...
async openAndDrawRectangleAOI() {
    ...
    const startX = box.x + Math.round(box.width * 0.30);
    const startY = box.y + Math.round(box.height * 0.30);
    const endX   = box.x + Math.round(box.width * 0.70);
    const endY   = box.y + Math.round(box.height * 0.60);
    
    await this.page.mouse.move(startX, startY);
    await this.page.mouse.down();
    // Increased steps to ensure Google Maps captures the drag gesture
    await this.page.mouse.move(endX, endY, { steps: 25 }); 
    await this.page.mouse.up();
    
    // Wait for the map to settle and the info window to appear
    await this.fastWait(2500); 

    // --- VALIDATE AND HIGHLIGHT DRAWN AOI ---
    await this.showStep('Validating drawn AOI is present on the map');
    
    // Using the updated helper that checks for InfoWindow
    ...
    if (!aoi) {
      ...
      await this.showStep(`AOI detected (type: ${aoi.type}, selector: ${aoi.selector || 'unknown'})`);
      logInfo(`AOI detected`, { type: aoi.type, selector: aoi.selector });
      
      // Highlight the detected element (could be InfoWindow or SVG)
      try { 
        await this.highlight(aoi.locator, { borderColor: 'rgba(255, 0, 0, 0.9)', pause: 1500 });
        await saveMapScreenshot(this.page, 'aoi', 'drawn_aoi_highlighted', false);
      } catch (e) { 
          addWarning('Failed to highlight AOI element: ' + e.message); 
      }
    }
  }
  ...
  async resetAOI() {
   ...
   async getMapZoomLevel() {
    try {
      ...
      addWarning('Could not get map zoom level: ' + e.message);
      return null;
    }
  }
