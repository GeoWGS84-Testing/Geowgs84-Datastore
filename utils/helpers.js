...
export async function showStep(page, text) {
  ...
  // This block executes if NEW errors or warnings occurred since the last step
  if (hasNewErrors || hasNewWarnings) {
    try {
      if (!page.isClosed()) {
        ...
        // 1. SHOW OVERLAY MESSAGE (Small Modal)
        await showFailureMessage(page, msgText, msgType, 0); 

        // 2. WAIT FOR VISUAL CONFIRMATION (1.5s)
        ...
        // 4. REMOVE OVERLAY MESSAGE (So test can continue)
  ...
  try {
    if (!page.isClosed()) {
      await page.evaluate(({ msg, type, bgColor, borderColor }) => {
        ...
        // Container: Centered, fixed width, NOT full screen
        const banner = document.createElement('div');
        ...
        Object.assign(banner.style, {
          ...
          fontSize: '16px', fontWeight: 'bold', padding: '25px', boxSizing: 'border-box', 
          textAlign: 'center',
          borderRadius: '8px',
...
export async function detectAOIOnMap(page) {
  try {
    // Strategy 1: Look for the specific InfoWindow generated by Google Maps Drawing Manager
    const infoWindow = page.locator('.gm-style-iw-c:has-text("area"), .gm-style-iw:has-text("area")');
    if (await infoWindow.count() > 0) {
        const bb = await getBoundingBoxForLocator(page, infoWindow.first());
        if (bb) return { type: 'aoi_info', locator: infoWindow.first(), bbox: bb, selector: 'infowindow' };
    }

    // Strategy 2: Look for SVG shapes (Standard Google Maps)
    const candidates = page.locator('#map svg path, #map svg rect, #map svg polygon');
    ...
    if (count > 0) {
        for (let i = count - 1; i >= 0; i--) {
            ...
            if (bb && bb.width > 30 && bb.height > 30) {
                return { type: 'aoi', locator: candidate, bbox: bb, selector: 'svg_shape' };
            }
        }
    }
  } catch (e) { console.warn('detectAOIOnMap error', e.message); }
  return null;
}
...
export async function waitForMapOverlayForScene(page, sceneId, type = 'any', timeout = 20000) {
  const start = Date.now();
  while (Date.now() - start < timeout) {
    try {
        if (type === 'preview' || type === 'any') {
            const imgLocator = page.locator(`#map img[src*="${sceneId}"], #map img[src*="browse"]`).first();
            if (await imgLocator.count() > 0) {
                const src = await imgLocator.getAttribute('src');
                if(src && !/marker|icon|pin|show\.png|hide\.png|preview\.png|details\.png/i.test(src)) {
                    return { type: 'preview', locator: imgLocator, bbox: await getBoundingBoxForLocator(page, imgLocator) };
                }
            }
        }
        ...
        if (type === 'outline' || type === 'any') {
             ...
             if (await svgLocator.count() > 0) {
                 ...
                 if (bb && bb.width > 30 && bb.height > 30) {
                     return { type: 'outline', locator: svgLocator, bbox: bb };
                 }
             }
        }
    } catch (e) {}
...
// --- Explicit Preview Image Detection ---
export async function waitForPreviewImageOnMap(page, sceneId, timeout = 20000) {
  ...
  while (Date.now() - start < timeout) {
    try {
        ...
        const genericImg = page.locator('#map img[src*="browse"], #map img[src*="maxar"]').first();
...
export default {
  ...
  getBoundingBoxForLocator, bboxIntersects, detectMapOverlayWithBBox, waitForPreviewImageOnMap, waitForMapOverlayForScene, getOutlineCoordsFromButton,
  detectAOIOnMap, waitForAOIOnMap,
  persistDiagnosticsSummary, sleep, getTestFileName, extractErrorReason, formatArtifactFilename, getElementMetrics, removeFailureMessage
}